...
6. **HTTP transport（本地完成，clean runner 待验收）**：chi、全局 request ID、严格 JSON/媒体类型、错误工具、完成日志与三类资源 HTTP CRUD 已接真实 repository；Service 创建与单项 GET 展开 Environment，集合与 PATCH 不展开。Environment CRUD、`service_id` 过滤与 cursor 分页已完成本地 HTTP 验证，仍需补 clean-runner smoke 后合入。

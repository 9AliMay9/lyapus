...
1. 在 `.github/workflows/verify.yml` 的 Service smoke 分组补充 PATCH 字段保留/显式 `null`、引用删除 409、无子资源删除 204 与删除后 404；本地检查 YAML/shell 后提交 PR。
2. PR required `verify`、`smoke` 与 `atlas-community` 全绿后更新最终证据并 squash merge；随后按同一分层实现 Environment 独立 CRUD 与 `service_id` 过滤。

...
1. 新建短生命周期功能分支，按既有 domain → repository/sqlc → service → HTTP 分层实现 Environment 独立 CRUD 与 `service_id` 过滤。
2. 为 Environment 补齐单元、真实 PostgreSQL integration、race、真实 HTTP 与 clean-runner smoke 证据，再进行同级收口审阅。

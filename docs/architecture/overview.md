...
LYAPUS_DATABASE_URL ─┘                                  ├──→ JSON slog ──→ stdout
                                                        ...
                                                        └──→ HTTP transport
                                                               ...
                                                               └──→ /v1/teams POST/GET, /v1/teams/{team_id} GET ──→ TeamService

internal/catalog domain ←── PostgreSQL repository adapter ←── sqlcgen
        （Team repository CRUD/稳定分页与业务服务已实现；创建、单项读取、列表读取端点已装配到 HTTP）
```
...
- `internal/catalog` 已有 Team domain、完整 repository 接口、PostgreSQL adapter 与 TeamService；当前已实现 CRUD、基于 `(created_at, id)` 的稳定分页，以及 `POST /v1/teams`、`GET /v1/teams/{team_id}`、`GET /v1/teams`。chi 与 HTTP DTO 留在 catalog transport，不进入 domain。

当前仍未实现 Team 的 PATCH/DELETE、Service/Environment repository 和 HTTP API、Compose、Kafka、OpenTelemetry、Kubernetes、前端或 AI 组件；这些内容不能画入已实现数据流。

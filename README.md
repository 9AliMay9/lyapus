...
M0（工程基线）已经完成。M1 正在施工：当前已有 PostgreSQL migration、`pgxpool` 启动与 readiness、sqlc 基线、Team repository CRUD/稳定游标分页及真实 PostgreSQL 集成测试，以及完整 Team HTTP CRUD（创建、单项/列表读取、PATCH、DELETE）。Service/Environment、Compose、查询计划实验仍未完成；当前也没有消息队列、OpenTelemetry、前端或 AI 功能。

...
7. **约束实证**：使用绕过 Go 校验的真实 SQL integration tests，补齐外键、唯一以及格式、长度、时间 `CHECK` 的成功/失败矩阵；不因测试缺口修改已经与契约一致的 schema。

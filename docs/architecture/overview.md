...
internal/catalog domain ←── PostgreSQL repository adapter ←── sqlcgen
        （Team Create/Get 已实现；尚未装配到 HTTP）
```
...
- `internal/catalog` 已有 Team domain、repository 接口和 PostgreSQL adapter；当前只有 Create/Get，尚无业务服务、catalog HTTP 路由或 `cmd/apiserver` 装配。

当前仍未实现 chi 路由、完整 CRUD、Compose、Kafka、OpenTelemetry、Kubernetes、前端或 AI 组件；这些内容不能画入已实现数据流。

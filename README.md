...
M0（工程基线）已经完成。M1 正在施工：当前已有 PostgreSQL migration、`pgxpool` 启动与 readiness、sqlc 基线，以及 Team、Service、Environment 三类资源的完整 HTTP CRUD。Environment 独立 CRUD 与 `service_id` 过滤已由 PR #23 以 squash commit `4b311b9` 合入 `main`。数据库约束测试已通过本地验证、收口审阅及三项 required CI，并由 PR #25 以 squash commit `51fd1d2` 合入 `main`。M1 v0.1 仍缺 Compose 空环境交付、查询计划实验，以及根 README 的数据模型、API 示例和五分钟演示路径。当前也没有消息队列、OpenTelemetry、前端或 AI 功能。

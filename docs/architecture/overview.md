...
internal/catalog domain ←── PostgreSQL repository adapter ←── sqlcgen
        （Team 与 Service CRUD、稳定分页、事务创建和业务服务已实现）
```
...
当前仍未实现 Environment 独立 repository/service/HTTP CRUD、Compose、Kafka、OpenTelemetry、Kubernetes、前端或 AI 组件；这些内容不能画入已实现数据流。Service mutation 已有本地验证，但在 required clean runner 通过前不视为已完成合并交付。

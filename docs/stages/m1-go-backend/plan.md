...
3. **schema 与 migrations（基线已完成）**：期望 schema、两份 versioned migration、空库与前滚路径已建立；PR #25 已补充直接 SQL 的字段 CHECK 插入边界、唯一性范围、外键和引用删除验证，通过本地及 required CI，并以 `51fd1d2` 合入 `main`。
